...
function renderProducts(items) {
    ...
    if (grid) {
        grid.innerHTML = items.map((product, index) => {
            ...
            const basePrice = (variants && variants[0]) ? variants[0].price : 0;
            ...
            <div class="bg-white rounded-lg overflow-hidden shadow-md card-hover indian-border bg-white reveal" style="animation-delay: ${index * 100}ms">
                <div class="relative h-56 overflow-hidden bg-gray-200 group cursor-pointer" onclick="window.openProductModal('${productIdString}')">
                ...
                <div class="p-4 text-center">
                    ...
                    <h3 class="font-serif text-lg font-bold text-gray-800 leading-tight hover:text-spice-red transition-colors cursor-pointer" onclick="window.openProductModal('${productIdString}')">${product.name}</h3>
                    ...
                    <div class="flex items-center justify-center gap-3 mt-2">
                        ...
                        <button onclick="event.stopPropagation(); window.openProductModal('${productIdString}')" class="bg-spice-red text-white px-4 py-2 rounded-full font-bold shadow-lg hover:scale-105 transition-transform flex items-center gap-1 text-sm">
    ...
    try {
        ...
        if (error) {
            ...
            if (savedOrder) {
                ...
                CartService.clear(); // Clear local/remote cart
            }
        }
    } catch (error) {
        console.error('Error saving order:', error);
...
function startPhonePeGatewayPayment() {
    ...
    // Simulation of Gateway Redirection/Verification
    setTimeout(() => {
        if (statusMsg) statusMsg.innerText = 'Verifying Payment Status with PhonePe...';

        const isSuccess = confirm(`[PHONEPE SECURE GATEWAY]\n\nMerchant: Sri Pickles\nAmount: ₹${snapshot.total.toFixed(2)}\n\nSelect Payment Outcome:\nOK = Payment Successful\nCancel = Payment Failed/Cancelled`);

        if (isSuccess) {
            if (statusMsg) {
                statusMsg.innerText = 'Payment Verified! Finalizing Order...';
                statusMsg.classList.remove('bg-blue-100', 'text-blue-700');
                statusMsg.classList.add('bg-emerald-100', 'text-emerald-700');
            }
            setTimeout(() => {
                const mockTxid = `PPGATEWAY${Date.now()}`;
                confirmPayment(mockTxid);
